...
def wavedata(url1, url2, url3):
    ...
    for i in [x, y, z]:
        if i.status_code == 200:
            jsonFormat = i.json()
            waveLength = jsonFormat['properties']['waveHeight']['values']
            waveLen = len(waveLength)
            array.append(waveLen)
            continue
        else:
            array.append(0)
            continue
    ...
    ##wave period
    wvpd = jsonOutput['properties']['wavePeriod']['values']
    ...
    ##wave direction
    wvdir = jsonOutput['properties']['waveDirection']['values']
    ...
    ##wind speed
    wnspd = jsonOutput['properties']['windSpeed']['values']
    ...
    ##wind direction
    wndir = jsonOutput['properties']['windDirection']['values']
    ...
    ## temp
    temp = jsonOutput['properties']['temperature']['values']
    ...
    ##chance of rain
    rainperc = jsonOutput['properties']['probabilityOfPrecipitation']['values']
